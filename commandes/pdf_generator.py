...
def generer_pdf_bon_commande(bon):
    ...
    lignes_data = [[
        ...
        Paragraph('<b>Référence client</b>',      style_normal),
        Paragraph('<b>Référence fournisseur</b>', style_normal),
        Paragraph('<b>Prix unitaire HTVA</b>',    style_normal),
        Paragraph('<b>Quantités</b>',             style_normal),
        Paragraph('<b>Total HTVA XOF</b>',        style_normal),
        Paragraph(f'<b>{delais_label}</b>',       style_normal),
    ]]
    ...
    lignes_table = Table(
        lignes_data,
        colWidths=[4.5*cm, 2.3*cm, 2.3*cm, 2.5*cm, 1.5*cm, 2.5*cm, 2.4*cm]
    )
    ...
    totaux_data = [
        ['Montant HTVA', f"{int(bon.total_ht):,} XOF".replace(',', ' ')],
    ]
