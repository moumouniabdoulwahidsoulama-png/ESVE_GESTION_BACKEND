...
def generer_pdf_facture(facture):
    ...
    style_small   = ParagraphStyle('small',   fontSize=8,  leading=11, textColor=colors.grey)
    ...
    client_info = (
        f'<b>{client.nom_entreprise}</b><br/>'
        + (f'{client.adresse}<br/>' if client.adresse else '')
        + (f'Tél : {client.telephone}<br/>' if client.telephone else '')
        + (f'N° RCCM : {client.rccm}<br/>' if client.rccm else '')
        + (f'N° IFU : {client.ifu}<br/>' if client.ifu else '')
        + (f"Régime : {client.regime_imposition}<br/>" if client.regime_imposition else '')
        + (f"Division fiscale : {client.division_fiscale}" if client.division_fiscale else '')
    )
    date_info = (
        f'<b>Date de la facture :</b> {facture.date_creation.strftime("%d/%m/%Y")}<br/>'
        f'<b>Validité :</b> {facture.validite_jours} Jours'
        + (f'<br/><b>Proforma origine :</b> {facture.proforma_origine.numero}' if facture.proforma_origine else '')
    )
    ...
    lignes_table = Table(lignes_data, colWidths=[5.5*cm, 2.5*cm, 2.8*cm, 2*cm, 3*cm, 2.2*cm])
    ...
    # ===== TOTAUX =====
    totaux_data = [
        ['Montant HT Brut',  f"{int(facture.total_ht_brut):,} XOF".replace(',', ' ')],
    ]
    if facture.remise_pct > 0:
        totaux_data.append([f'Remise ({facture.remise_pct}%)', f"- {int(facture.montant_remise):,} XOF".replace(',', ' ')])
        totaux_data.append(['Montant HT Net',   f"{int(facture.total_ht):,} XOF".replace(',', ' ')])
    totaux_data += [
        ['TVA (18%)',         f"+ {int(facture.tva_18pct):,} XOF".replace(',', ' ')],
        ['Retenue (5%)',      f"- {int(facture.retenue_5pct):,} XOF".replace(',', ' ')],
        ['BIC (2%)',          f"- {int(facture.bic_2pct):,} XOF".replace(',', ' ')],
        ['TOTAL NET',         f"{int(facture.total_net):,} XOF".replace(',', ' ')],
    ]
    ...
    # Couleur TVA
    tva_idx = next(i for i, r in enumerate(totaux_data) if 'TVA' in r[0])
    totaux_style.append(('TEXTCOLOR', (0, tva_idx), (-1, tva_idx), BLEU))
    # Couleur retenue et BIC
